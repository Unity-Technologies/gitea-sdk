...
func TestMilestones(t *testing.T) {
	...
	// ListRepoMilestones
	ml, resp, err := c.ListRepoMilestones(repo.Owner.UserName, repo.Name, ListMilestoneOption{})
	assert.NoError(t, err)
	assert.Len(t, ml, 2)
	assert.False(t, resp.Next())
	ml, resp, err = c.ListRepoMilestones(repo.Owner.UserName, repo.Name, ListMilestoneOption{State: StateClosed})
	assert.NoError(t, err)
	assert.Len(t, ml, 1)
	assert.False(t, resp.Next())
	ml, resp, err = c.ListRepoMilestones(repo.Owner.UserName, repo.Name, ListMilestoneOption{State: StateAll})
	assert.NoError(t, err)
	assert.Len(t, ml, 3)
	assert.False(t, resp.Next())
	ml, resp, err = c.ListRepoMilestones(repo.Owner.UserName, repo.Name, ListMilestoneOption{State: StateAll, ListOptions: ListOptions{PageSize: 2}})
	assert.NoError(t, err)
	assert.Len(t, ml, 2)
	assert.True(t, resp.Next())
	ml, resp, err = c.ListRepoMilestones(repo.Owner.UserName, repo.Name, ListMilestoneOption{State: StateAll, Name: "V3.0"})
	assert.NoError(t, err)
	assert.Len(t, ml, 1)
	assert.False(t, resp.Next())
